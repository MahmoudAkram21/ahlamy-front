...
export function AudioPlayer({ audioUrl, className = "" }: AudioPlayerProps) {
  const fullUrl = audioUrl.startsWith("http")
    ? audioUrl
    : `https://b-ahlamy.developteam.site${audioUrl}`;

  return (
    <div
      className={`flex items-center gap-3 rounded-2xl border border-sky-100 bg-gradient-to-r from-sky-50 to-purple-50 p-4 ${className}`}
    >
